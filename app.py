...
class App:
    ...
    def auto_predict_toggle(self):
        self.auto_predict = not self.auto_predict    #this will word as a toggle button 

    def save_for_class(self,class_num):              #when first btn is press the classname will be 1
        ret,frame=self.camera.get_frame()            #get the frame same as screenshot
        if not os.path.exists('1'):                  #first it check if path is exist or not,,,this will create the folder if not created earlier or if is there then it will skip this loop and pass to other 
            os.mkdir('1')
        if not os.path.exists('2'):
            os.mkdir('2')
        
        cv.imwrite(f"{class_num}/frame{self.counters[class_num-1]}.jpg",cv.cvtColor(frame,cv.COLOR_RGB2GRAY))   #this will write the frame in it
        img=PIL.Image.open(f"{class_num}/frame{self.counters[class_num-1]}.jpg")                                #open the image
        img.thumbnail((150,150),Image.Resampling.LANCZOS)                                                       #this is use to resize the image to 150*150 and lanczos is used for maintaining the quality same this will downgrade it with perfect ratio and quaility not changes
        img.save(f"{class_num}/frame{self.counters[class_num-1]}.jpg")                                          #after changes make in img the image gets saved

        self.counters[class_num-1]+=1                                                                           #increment to save the image next
